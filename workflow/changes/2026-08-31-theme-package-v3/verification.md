...
- `cargo test -p doubao-skin-desktop ui_regression_tests --no-fail-fast` — 15/15 passed. Target switches filter unsupported installed/store themes, refresh target previews, distinguish `专属适配` / `共享适配` / `兼容模式`, and cover target-scoped session operations.
- Multi-target live-session regression was reproduced before the fix with `cargo test -p doubao-skin-desktop applying_to_a_second_target_preserves_the_first_target_session -- --nocapture`: the second target replaced the only session slot and the test failed because WorkBuddy was no longer active. The desktop now owns `Applying / Active / Restoring` state, watcher handle, and generation per target; target selection does not stop or restore another target, while same-target apply/restore is serialized. The original regression plus same-target replacement, restore/reapply exclusion, and cross-target completion-generation tests pass. `./scripts/check.sh all` passes with 15 desktop tests, 48 skin-core tests, 9 authoring tests, 3 bundled-path tests, 4 CLI tests, 12 v3 contract tests, 2 schema tests, examples, clippy, 16 Web tests, TypeScript, the 42-page Next build, and dependency audit.
- The review regression for `targets.<id>.variants.<appearance>.css` first failed with `Shared` instead of `Tailored`; `target_has_substantive_delta` now checks CSS in the complete target layer, and `cargo test -p skin-core --test theme_package_v3 target_variant_css_counts_as_tailored_support -- --nocapture` passes.
...
### Desktop app — current review repair

- `./scripts/package.sh desktop-macos` rebuilt the current arm64 ad-hoc test App/ZIP/DMG successfully. The packaged `dist/豆皮.app` was launched by absolute path and inspected in its real 1120 × 720 fixed window: the three-target selector, installed-theme list, WorkBuddy preview, support label, opacity/detail area, “恢复默认”, and “应用主题” controls were visible without clipping or layout movement. The temporary app was then closed; this inspection did not apply or restore a theme in any official host.
- The desktop window remains intentionally non-resizable at 1120 × 720, so this repair adds no new narrow-window evidence and does not close the existing narrow-window/fresh-verdict residual Gate.
...
- Multi-target switching passed in a uniquely identified ad-hoc QA app built from the repaired desktop binary. `gallery-whale-maid` was applied first to DoubaoWork and then to WorkBuddy. While WorkBuddy was selected, loopback probes simultaneously reported `9222 -> {skin: gallery-whale-maid, target: doubao-work, style: true}` and `9224 -> {skin: gallery-whale-maid, target: workbuddy, style: true}`. Switching back to DoubaoWork and then back to WorkBuddy left each target's action labeled `正在使用`; a second simultaneous probe returned the same two independent markers.
- Targeted restore isolation also passed: restoring only WorkBuddy cleared its skin/target/style marker while DoubaoWork continued to report `gallery-whale-maid / doubao-work / style: true`. The remaining DoubaoWork injection was then removed with the normal CLI restore path; all three responsive DoubaoWork pages restored and the final chat probe returned `skin: null, target: null`. No screenshot or page text was captured for this state-only regression.
...
- After the successful simultaneous and targeted-restore probes, an extra attempt to reapply WorkBuddy crossed its existing explicit restart gate but the host reopened without the `9224` listener. WorkBuddy had already been verified unthemed before that attempt, and DoubaoWork was restored afterward. This host relaunch anomaly is separate from the fixed cross-target eviction path and is not counted as a successful WorkBuddy reapply.
