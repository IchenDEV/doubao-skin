...
- Multi-target live-session regression was reproduced before the fix with `cargo test -p doubao-skin-desktop applying_to_a_second_target_preserves_the_first_target_session -- --nocapture`: the second target replaced the only session slot and the test failed because WorkBuddy was no longer active. The desktop now owns one watcher/session/generation slot per target; target selection does not stop or restore another target, while reapplying or restoring still stops only the selected target. The original regression plus same-target replacement/generation isolation tests pass, and `./scripts/check.sh rust` passes with 13 desktop tests, 48 skin-core tests, 9 authoring tests, 3 bundled-path tests, 4 CLI tests, 11 v3 contract tests, 2 schema tests, examples, and clippy.
