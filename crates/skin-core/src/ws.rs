//! Minimal synchronous WebSocket client + Chrome DevTools Protocol caller.
//! Client frames are masked per
...
impl WebSocket {
    ...
    pub fn recv_text(&mut self, timeout: Duration) -> Result<String, String> {
        ...
        loop {
            ...
            match opcode {
                0x1 => {
                    return String::from_utf8(payload).map_err(|e| format!("invalid utf-8: {e}"));
                }
...
impl Cdp {
    pub fn connect(ws_url: &str, timeout: Duration) -> Result<Self, String> {
        Ok(Cdp {
            ws: WebSocket::connect(ws_url, timeout)?,
            next_id: 0,
        })
    }
