//! macOS offline build pipeline:
//! clone DoubaoWork.app -> inject theme -> re-sign.
//!
//! Why a clone? The official app bundle is protected by macOS App
...
fn skin_app_in_home(home: Option<PathBuf>) -> Result<PathBuf, String> {
    home.map(|directory| directory.join("Applications/DoubaoWork-Skin.app"))
        .ok_or_else(|| "无法确定当前用户目录，未创建或删除离线克隆".into())
}

pub fn skin_app() -> Result<PathBuf, String> {
    skin_app_in_home(dirs::home_dir())
}
...
fn gzip_mtime0(raw: &[u8]) -> Vec<u8> {
    let mut enc: GzEncoder<Vec<u8>> = GzBuilder::new()
        .mtime(0)
...
pub fn apply<F: FnMut(String)>(theme: &Theme, mut log: F) -> Result<PathBuf, String> {
    let skin = skin_app()?;
    let source = TargetApp::DoubaoWork
        .installed_app_bundle()
        .ok_or_else(|| "未找到系统注册的豆包工作应用".to_string())?;
    if skin.exists() {
    ...
    log(format!(
        "  injected into {} pak-embedded pages",
        patched.len()
    ));
    ...
    log(
        "first launch will ask for keychain access to 'DoubaoWork Safe Storage' \
...
mod tests {
    ...
    #[test]
    fn offline_build_requires_a_real_home_directory() {
        assert_eq!(
            skin_app_in_home(Some(PathBuf::from("/Users/tester"))).unwrap(),
            PathBuf::from("/Users/tester/Applications/DoubaoWork-Skin.app")
        );
        assert_eq!(
            skin_app_in_home(None).unwrap_err(),
            "无法确定当前用户目录，未创建或删除离线克隆"
        );
    }
}
